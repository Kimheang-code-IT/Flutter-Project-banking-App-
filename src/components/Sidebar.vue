      ...
      <ul class="space-y-2">
        <li v-for="item in menuItems" :key="item.path" :class="[isCollapsed ? 'flex justify-center' : '']">
          <router-link :to="item.path" :class="[
            'flex items-center rounded-sm transition-colors relative cursor-pointer',
            isCollapsed ? 'justify-center px-6 py-3 w-full' : 'px-4 py-3',
            'hover:bg-gray-100 dark:hover:bg-gray-700',
            $route.path === item.path
              ? 'bg-blue-50 dark:bg-blue-900/30 font-semibold text-blue-600 dark:text-blue-400 border-l-4 border-blue-600 dark:border-blue-400'
              : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100'
          ]" :title="isCollapsed ? item.label : ''">
            <span class="w-5 h-5 flex-shrink-0 flex items-center justify-center pointer-events-none"
              :class="{ 'mr-3': !isCollapsed }" v-html="item.icon"></span>
            <span v-if="!isCollapsed" class="pointer-events-none">{{ item.label }}</span>
          </router-link>
        </li>
      </ul>
    </nav>
